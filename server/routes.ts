...
app.get('/api/nfts/user', async (req: Request, res: Response) => {
 ...
 const auction = nfts.filter(n => n.status === 'auction');
  ...
  app.delete('/api/nfts/:id', async (req: Request, res: Response) => {
    try {
      ...
      if (nft.imagePublicId) {
        await deleteFromCloudinary(nft.imagePublicId);
      }

      await Collection.findByIdAndUpdate(nft.collectionId, { $inc: { nftCount: -1 } });
